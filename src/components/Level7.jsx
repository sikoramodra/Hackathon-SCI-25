...
const decodeMap = Object.fromEntries(Object.entries(scrambleMap).map(([k, v]) => [v, k]));
...
const dyslexiaTransform = (text) => {
	return text
		...
		.map((word) => {
			...
			const replacements = { b: 'd', d: 'b', p: 'q', q: 'p', m: 'n', n: 'm', w: 'v', v: 'w' };
			word = word
				.split('')
				.map((l) =>
...
export default function DyslexiaLevel({ complete }) {
	...
	return (
		<div className='min-h-screen flex items-center justify-center bg-blue-50'>
			...
			<div
				className='max-w-xl w-full p-8 rounded-xl shadow-lg relative'
				style={{
					backgroundImage: `
				...
				<h2
					className='text-2xl mb-8 text-blue-900'
				...
				<p
					className='text-gray-900 mb-4 leading-8'
					style={{ fontFamily: 'Patrick Hand, cursive' }}>
					Spróbuj wpisać hasło "dysleksja", mimo że każda litera klawiatury daje inną
					literę.
				</p>

				<input
					className={`w-full border border-gray-400 rounded-lg py-3 px-4 text-lg text-center mb-4 focus:outline-none focus:ring-2 focus:ring-blue-400 transition`}
					value={displayValue}
  ...
  ${wrong ? 'bg-red-600' : 'bg-blue-600 hover:bg-blue-700'}`}
					style={{ fontFamily: 'Patrick Hand, cursive' }}>
					Zatwierdź
				</button>

				<p
					className='mt-4 text-sm text-gray-500 leading-6'
					style={{ fontFamily: 'Patrick Hand, cursive' }}>
					*Tak osoby z dysleksją mogą odbierać tekst: litery przeskakują, zlewają się,
					czasem znikają.
				</p>
			</div>
		</div>
